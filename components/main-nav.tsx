  ...
  return (
    <header className={cn(
      ...
      <div className="mx-auto flex h-[4.2rem] max-w-[88rem] items-center px-5 sm:px-8 lg:px-12">
        ...
        {/* Interactive Desktop Navigation with Hover Indicator */}
        <nav 
          className="hidden items-center gap-1.5 md:flex relative p-1.5 rounded-full bg-muted/40 border border-border/50 shadow-inner" 
          ...
          {routes.map((route, index) => {
            ...
            return (
              <Link 
                ...
                className={cn(
                  "relative z-10 px-4 py-1.5 text-xs font-medium transition-colors duration-200 rounded-full",
                  route.active 
              ...
              >
                {route.label}
                {route.active && hoveredIndex === null && (
                  <span className="absolute inset-0 bg-background shadow-xs border border-border/60 rounded-full -z-10 animate-in fade-in zoom-in-95 duration-200" />
                )}
                {isHovered && (
                  <span className="absolute inset-0 bg-accent/60 rounded-full -z-10 transition-all duration-150 ease-out" />
                )}
              </Link>
            )
          })}
